      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 输入框的blur事件，是失去焦点事件，只要失去焦点就会触发这个事件
              ...
              <div class="add">
                ...
                比如从home跳转到search  从search跳转到详情，跳转时不需要发请求，而是跳转过去后再发请求拿数据

                而加入购物车不一样，当我们点击添加购物车的时候，我们得需要先发请求给后台，后台需要把这个
                购物车信息存储到数据库，请求成功之后，会返回给我们信息。我们根据这个信息再去跳转
                否则，有可能后台添加数据库失败，而我们已经跳到下个页面，就会出问题-->
...
<script>
  ...
  export default {
    ...
    // 这里是为了把路由当中的参数，处理存储在data内部，后期使用这个数据就方便了
    beforeMount() {
    ...
    methods: {
      ...
      changeChecked(spuSaleAttrValueList, spuSaleAttrValue) {
        // spuSaleAttrValueList 代表当前属性的所有属性值列表
      ...
      async addShopCart() {
        ...
        //第一种写法中：action的异步函数，返回的promise永远是成功的，要看action异步函数的return 【不推荐】
        ...
        //第二种写法中：action的异步函数，返回的promise可能成功也可能失败，成功就代表请求成功，返回结果ok
        ...
        try {
          ...
          //第二步：根据请求添加购物车的返回信息决定是否跳转到添加购物车成功页面
          //请求成功就跳转到添加购物车成功页面
          alert('添加购物车成功，准备去往成功页面')
          // 往添加购物车成功页面跳转的时候，需要带两个参数，分别是：skuNum和商品详情信息
          // 其中skuNum是一个简单数据，我们直接通过路由传参query参数带过去
          // 但是由于商品详情信息是一个复杂的数据，最好不要通过路由传参传递对象，因此我们采用本地存储方案，sessionStorage
