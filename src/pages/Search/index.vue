        ...
        <div class="details clearfix">
          ...
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              ...
              //3、图标是向上还是向下  和数据排序类型相关 asc、desc
              -->
              <ul class="sui-nav">
                <!-- <li
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        // 这个对象用来初始化所有的搜索参数
        // 今后只要是搜索条件的所有相关数据，全部都在这个对象内部初始化好
        searchParams: {
          category1Id: '',
          category2Id: '',
          category3Id: '',
    ...
    methods: {
      // 发送请求获取search页面动态数据
      getSearchData() {
        // dispatch 如果要传递多个参数，那么多个参数必须构成一个对象去传递
        // 也就是说，dispatch只能传递一个参数
        // this.$store.dispatch('search/getSearchInfo', {})  // 这里刚开始传空对象只是为了获取数据展示页面
        // 我们点击三级分类或者点击搜索按钮跳到search页面时，就已经有搜索条件了
        // 点击三级分类跳转，那么搜索条件就应该是三级分类id和分类名称
        // 而点击搜索按钮跳转，那么搜索条件就应该是自己输入的关键字
        // 所以这个请求，参数不应该是空对象
        this.$store.dispatch('search/getSearchInfo', this.searchParams)
      },
      // 处理搜索参数
      handlerSearchParams() {
        // 在点击三级分类或者点击搜索按钮跳过来之前，把对应的三级分类名称和id或者关键字keywords，拿到
        // 添加到data当中searchParams对应的搜索项中
        ...
        let searchParams = {
          ...
          categoryName,
          keyword,
        } // 这样可以保证searchParams里面一定包含了用户点击传递过来的搜索条件，没有就是undefined
        ...
        // forEach 是数组的一个方法，主要是用来遍历数组的，效率最高，但是不可以使用continue和break
        // for..of 是es6里面新加的一种遍历方法（前提必须是可迭代对象），效率没有forEach高（比其它的要高），也可以使用
        //可以使用continue和break，缺点是for..of只能针对可迭代对象进行遍历

        //遍历对象最快的方法也是使用forEach 是把对象的属性转化为数组然后进行遍历
      ...
      removeCategoryName() {
        ...
        // 一旦改变搜索条件，则需要让pageNo从1开始重新发送请求
        this.searchParams.pageNo = 1
        // this.getSearchData()
        // 这里删除以后，不会动我的原来路径，所以这样发请求不行，我们得让路径发生变化
        // 然后发送请求的时候，删除对应的参数
        this.$router.replace({ name: 'search', params: this.$route.params }) // 这个目的是让路径变化
        // 为什么路径变化了就发送请求了呢？而且参数也对了呢？
        // 是因为这里发送请求依赖的是监听器里面的代码（监视了$route的改变）
      },
      // 删除关键字搜索条件，重新发送请求
      removeKeyword() {
      ...
      searchForTrademark(trademark) {
        // trademark 最终参数的样式要去参考接口文档，如 ID:小米
        this.searchParams.trademark = `${trademark.tmId}:${trademark.tmName}`
        // 一旦改变搜索条件，则需要让pageNo从1开始重新发送请求
        this.searchParams.pageNo = 1
        this.getSearchData()
      },
      // 用户点击平台属性之后，根据平台属性搜索重新发送请求
      searchForProps(attrValue, attr) {
        // 接口文档对props的格式要求：商品属性的数组 ["属性ID:属性值:属性名"]
        let prop = `${attr.attrId}:${attrValue}:${attr.Name}`
        // 判断数组当中1是否已经存在当前这个属性，如果有了，就不要再去发送请求了
        // 数组方法（高级方法）：every、some、reduce、filter、map
        let isRepeate = this.searchParams.props.some((item) => item === prop)
        if (isRepeate) {
          // 进入判断则证明存在这个属性，发过请求了，就别再继续发了
          return
        }
        // 一旦改变搜索条件，则需要让pageNo从1开始重新发送请求
        this.searchParams.pageNo = 1
      ...
      changeSort(sortFlag) {
        ...
        if (sortFlag === originSortFlag) {
          // 如果用户点击的排序标志和原来的是一样的，证明点击的还是同一个排序，那么我们需要把排序类型改变
          newOrder = `${originSortFlag}:${
            originSortType === 'asc' ? 'desc' : 'asc'
          }`
        } else {
          // 如果用户点击的排序标志和原来的不一样，证明点击的不是同一个排序，那么我们需要把排序标志改变，排序类型默认
          newOrder = `${sortFlag}:desc`
        }
        this.searchParams.order = newOrder // 把排序规则的数据修改
        // 一旦改变搜索条件，则需要让pageNo从1开始重新发送请求
        this.searchParams.pageNo = 1
        this.getSearchData() // 重新发送请求，获取新排序好的数据
      },
