...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //点击立即支付的回调
      async pay() {
        //第一步：根据支付信息当中的codeUrl生成二维码进行弹框展示
        try {
          //生成二维码图片链接成功
          ...
          this.$alert(`<img src="${imgUrl}" />`, '请使用微信扫码支付', {
            ...
            center: true,
            // beforeClose一旦被设定，那么消息盒子是否关闭，我们可以在这个里面进行控制
            beforeClose: (action, instance, done) => {
              //action代表用户点击的是哪个按钮 'confirm确定按钮'、'cancel取消'、'close关闭'；
              if (action === 'confirm') {
                // 正常的代码逻辑
                // 代表用户点击的是确定
                // 判断如果没有支付
          ...
          // .then() 对应点击确定按钮之后的操作
          // .catch() 对应点击取消按钮之后的操作
          // 如果采用这两个去操作，都会强制关闭msgBox弹出框
          // 而我们现在并不是要直接关闭，o而是需要判断用户是不是支付了，才决定关闭不关闭

          // 轮询：隔2秒发一个请求，为了让后台返回当前订单的支付状态
          // 用以判断用户到底是支付了还是没支付
          if (!this.timer) {
            ...
            this.timer = setInterval(async () => {
              ...
              if (result.code === 200) {
                ...
                //3、把定时器清除（一定要把定时器彻底清除）
                clearInterval(this.timer) //clearInterval只是停止了定时器，不让管理模块当中定时器生效
                this.timer = null // 我们还需要把之前设置在vm身上的的定时器的编号id 也清除
                //4、自动跳转到支付成功页面
                //跳转之前要关闭掉弹出框
