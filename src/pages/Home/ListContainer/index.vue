...
<script>
  ...
  // import 'swiper/css/swiper.css'  // 引入swiper的css，如果多个组件要用，可以在main里面引入
  ...
  export default {
    ...
    // },
    mounted() {
      this.$store.dispatch('home/getBannerList') //actions中执行的是异步代码

      // 挂载完成后去实例化swiper
      // 这样是不行的，因为实例化时，页面显示还不一定成功
      // 按道理来说挂载完毕，页面的dom结构就算初始化形成完成，即初始化数据是没问题的，在此去实例化应该是可以的
      // 但是，这个页面轮播图结构中的swiper-slide，是根据请求回来的数据，动态创建生成的
      // 所以，我们必须得保证数据请求回来之后，再去实例化，有了数据，slide的div才会动态创建
